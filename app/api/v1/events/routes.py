...
async def search_events(criteria: SearchModel, service: EventService = Depends(get_event_service)):
    return await service.search_events(criteria.event)

@events_router.put("/events/{event_id}")
async def update_event(event_id: int, event: EventUpdate, service: EventService = Depends(get_event_service)):
